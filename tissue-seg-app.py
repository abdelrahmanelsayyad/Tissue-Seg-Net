...
def create_upload_section_auto():
    """Create responsive upload section with automatic mobile detection"""
    
    # Get mobile status automatically
    mobile_mode = get_mobile_status()
    
    # Optional: Show detection result for debugging (remove in production)
    if st.checkbox("🔧 Show Device Detection (Debug)", value=False):
        device_type = "📱 Mobile Device" if mobile_mode else "🖥️ Desktop Device"
        st.info(f"**Detected:** {device_type}")
        
        # Manual override for testing
        manual_override = st.radio(
            "Manual Override (for testing):",
            options=["Auto-detect", "Force Mobile", "Force Desktop"],
            index=0
        )
    ...
    if mobile_mode:
        ...
        st.markdown("### 📱 Mobile Upload")
        st.caption("🔍 Mobile device detected - optimized interface loaded")
        ...
        # Camera capture
        camera_image = create_mobile_camera_interface()
        
    else:
        # Desktop layout - automatic
        st.markdown("### 🖥️ Desktop Upload")
        st.caption("🔍 Desktop device detected - full interface loaded")
